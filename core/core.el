...
(unless noninteractive (setq file-name-handler-alist nil))
...
(defconst doom-private-dir "~/.emacs.d/private/")
;; (defconst doom-private-dir
;;   (if-let (doomdir (getenv "DOOMDIR"))
;;       (expand-file-name (file-name-as-directory doomdir))
;;     (or (let ((xdgdir (expand-file-name "doom/" (or (getenv "XDG_CONFIG_HOME") "~/.config"))))
;;           (if (file-directory-p xdgdir) xdgdir)) "~/.doom.d/"))
...
(setq auth-sources (list (expand-file-name "authinfo.gpg" doom-etc-dir) "~/.authinfo.gpg"))
