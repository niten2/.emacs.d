...
(when (featurep! +everywhere)
  ...
  (appendq! evil-collection-key-blacklist
            (append
             ;; (when (featurep! :tools lookup) '("gd" "gf" "K"))
             (when (featurep! :tools lookup) '("gd" "gf"))
             (when (featurep! :tools eval) '("gr" "gR")) '("[" "]" "gz" "<escape>")
             )
            )
...
(map!
 ...
 ;; :n "M-q" 'kill-buffer-and-window

 :n "t k" 'awesome-tab-forward
 :n "t j" 'awesome-tab-backward
 :n "t h" 'awesome-tab-select-beg-tab
 :n "t l" 'awesome-tab-select-end-tab
 :n "t g" 'awesome-tab-switch-group
 :n "t d" #'kill-current-buffer
 :n "t m h" 'awesome-tab-move-current-tab-to-beg
 :n "t m j" 'awesome-tab-move-current-tab-to-left
 :n "t m k" 'awesome-tab-move-current-tab-to-right
