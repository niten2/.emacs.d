(after! projectile (pushnew! projectile-project-root-files "main.py" "setup.py" "requirements.txt"))

;; (defvar +python-ipython-repl-args '("-i" "--simple-prompt" "--no-color-info")
;;   "CLI arguments to initialize ipython with when `+python/open-ipython-repl' is called."
;; )

;; (defvar +python-jupyter-repl-args '("--simple-prompt")
;;   "CLI arguments to initialize 'jupiter console %s' with when `+python/open-ipython-repl' is called."
;; )
...
;;   :defer t
;;   :init
;;   (setq python-environment-directory doom-cache-dir python-indent-guess-indent-offset-verbose nil)
...
(use-package! py-yapf
  ...
  (map! :map python-mode-map
        :localleader
        :desc "py-yapf-buffer"    "f" #'py-yapf-buffer
  )
)
